...
class PositionLayer(tf.keras.layers.Layer):
   ...
   def call(self, x):
      ...
      mask = K.expand_dims(K.cast(K.arange(start =0, stop= K.shape(x)[1]), 'float32'), axis=-1);
...
class SelfLayer(tf.keras.layers.Layer):
    ...
    def call(self, inputs):
        ...
        A = tf.keras.backend.batch_dot(Q, tf.transpose(K, (0,2,1))) / self.denom;
